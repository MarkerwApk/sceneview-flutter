...
@freezed
abstract class ARSceneConfig with _$ARSceneConfig {
  const factory ARSceneConfig({
    ...
    @Default(LightEstimationMode.environmentalHDR) LightEstimationMode lightEstimationMode,
    @Default(DepthMode.automatic) DepthMode depthMode,
    @Default(InstantPlacementMode.localYUp) InstantPlacementMode instantPlacementMode,
    @Default(false) bool cloudAnchorEnabled,
    @Default(false) bool geospatialEnabled,
  }) = _ARSceneConfig;

  factory ARSceneConfig.fromJson(Map<String, dynamic> json) => _$ARSceneConfigFromJson(json);
  ...
  Map<String, dynamic> toJson() => {
    ...
    'lightEstimationMode': lightEstimationMode.toString().split('.').last.toUpperCase(),
    'depthMode': depthMode.toString().split('.').last.toUpperCase(),
    'instantPlacementMode': instantPlacementMode.toString().split('.').last.toUpperCase(),
    'cloudAnchorEnabled': cloudAnchorEnabled,
    'geospatialEnabled': geospatialEnabled,
    'planeRenderer': {'isEnabled': planeRendererEnabled, 'isVisible': planeRendererEnabled},
  };
}
